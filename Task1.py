...
def scrape_halaman(soup, page_url):
    """
    Mengambil data semua buku dari satu halaman menggunakan BeautifulSoup object.
    ...
    for buku in buku_list:
        # Judul dan URL buku
        judul_elem = buku.h3.a
        judul = judul_elem['title']
        url_relative = judul_elem['href']
        url_buku = page_url.rsplit('/', 1)[0] + '/' + url_relative if not url_relative.startswith('http') else url_relative
...
while current_url:
    ...
    data_halaman = scrape_halaman(soup, current_url)
    semua_data.extend(data_halaman)

    print(f"    └─ Ditemukan {len(data_halaman)} buku")
...
print(df[['Judul', 'Harga (£)', 'Rating (1-5)', 'Stok']].head().to_string(index=False))
